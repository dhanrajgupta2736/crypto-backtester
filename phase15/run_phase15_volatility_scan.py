...
def main():
    # Research universe definitions for Phase 15A
    modes = ['VE_1_BB_SQUEEZE', 'VE_2_KELTNER_SQUEEZE', 'VE_3_ATR_EXPANSION']
    ...
    print(f"Starting Phase 15A Volatility Expansion Scan: running {total_runs} combinations...")
    ...
    df_results.to_csv("results/phase15_volatility_scan.csv", index=False)
    df_results.to_csv("phase15_volatility_scan.csv", index=False)
    print(f"\nSaved all results to results/phase15_volatility_scan.csv and phase15_volatility_scan.csv")

    # 8. Summary Report Generation
    print("\nGenerating phase15_summary.txt...")
    summary_path = "results/phase15_summary.txt"
    
    # Calculate best metrics (preventing division-by-zero or idxmax on empty)
    valid_df = df_results[df_results['trades'] > 0]
    
    best_pf = valid_df.loc[valid_df['pf'].idxmax()] if not valid_df.empty else None
    best_sharpe = valid_df.loc[valid_df['sharpe'].idxmax()] if not valid_df.empty else None
    best_exp = valid_df.loc[valid_df['expectancy'].idxmax()] if not valid_df.empty else None
    ...
    summary_content.append("  PHASE 15A VOLATILITY EXPANSION SCAN SUMMARY")
    summary_content.append("=" * 80)
    ...
    summary_content.append("-" * 80)
    for coin in coins:
        df_c = valid_df[valid_df['coin'] == coin]
    ...
    with open("phase15_summary.txt", 'w') as f:
        f.write("\n".join(summary_content))
        
    print(f"Summary written to {summary_path} and phase15_summary.txt")
